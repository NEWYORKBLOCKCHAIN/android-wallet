    ...
    public void getAuthentication(AuthenticationCallback authCallback, @NonNull Activity activity, Operation callbackId)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) // 30+
        {
            if (!hasStrongBiometric && !hasDeviceCredential)
            {
                ...
                Toast.makeText(activity.getBaseContext(), "Set a screen lock on your mobile and then you will see a seed phrase",Toast.LENGTH_LONG).show();
        ...
        else
        {
            ...
            {
                // Toast.makeText(activity.getBaseContext(), "ERROR2",Toast.LENGTH_LONG).show();
                showAuthenticationScreen(activity, authCallback, callbackId);
                return;
            }
            else
        ...
        {
            //  Toast.makeText(activity.getBaseContext(), "ERROR3",Toast.LENGTH_LONG).show();
