...
public class DetailTokenHolder extends BinderViewHolder<TokenCardMeta> implements View.OnClickListener, View.OnLongClickListener {
    ...
    public DetailTokenHolder(ViewGroup parent, AssetDefinitionService assetService, TokensService tSvs, Realm r)
    {
        ...
        bsc = findViewById(R.id.textView4);
...
//            tokenLayout.setBackgroundResource(R.drawable.background_marketplace_event);
            ...
            bsc.setText(token.getShortSymbol());
...
//            contractType.setVisibility(View.GONE);
    ...
    private void setTickerInfo(TokenTicker ticker)
    {
       ...
       // char d = Character.highSurrogate(Integer.parseInt(token.getStringBalance()));
    //    Sň x1 =  ((token.getStringBalance())*(ticker.price));
      //  Double xxx = x1*0.01;



        String check = token.getStringBalance();
        String s1 = removeSpace(check);


        String result = s1.replace( ',', ' ');
        String s = removeSpace(result);
            ...
            double myValue = Double.parseDouble(s);
                ...
                String x = String.format("%.10f", myValue);

        //    String x = String.format("%.10f", myValue);

            double x1 = Double.parseDouble(String.valueOf(myValue));


            double x12 = Double.parseDouble(ticker.price);
            double x13 = Double.parseDouble(String.valueOf(x1*x12));
            String x14 = String.format("%.3f", x13);

            TextView textView = findViewById(R.id.textView7);
            textView.setText(String.valueOf(x14) + " " +ticker.priceSymbol);
  ...
  //      }
     ...
     //    textView.setText(String.valueOf((token.getStringBalance())));
      //   textView.setText(String.valueOf(ticker.price));
