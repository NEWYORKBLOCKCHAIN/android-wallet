...
public class NewSettingsFragment extends BaseFragment {
    ...
    private SettingsItemView showSeedPhrase2;
    ...
    private void initializeSettings(View view) {
        ...
        showSeedPhrase = new SettingsItemView.Builder(getContext())
          ...
          showSeedPhrase2 = new SettingsItemView.Builder(getContext())
                .withIcon(R.drawable.theme_icon)
                  .withType(SettingsItemView.Type.TOGGLE)
                  .withTitle(R.string.theme_ľ)
                .withListener(this::onShowSeedPhrase2) //onShow
                .build();


        SharedPreferences prefs = getActivity().getSharedPreferences("Theme", Context.MODE_PRIVATE);
        String lanSettings = prefs.getString("theme", null);

        if (lanSettings != null) {
          if (lanSettings.equals("day")){
            showSeedPhrase2.setToggleState(false);
         } else {
            showSeedPhrase2.setToggleState(true);
         }
        } else {
   ...
   /*     biometricsSetting =
    ...
    private void addSettingsToLayout() {
      ...
      //  walletSettingsLayout.addView(showSeedPhrase2, walletIndex++);
...
//        if (CustomViewSettings.getLockedChains().size() == 0)
    ...
    private void onShowSeedPhrase2()
    {
        if (showSeedPhrase2.getToggleState()){
            ...
            editor.commit();
            Toast.makeText(getContext(),"Set up theme ....",Toast.LENGTH_LONG).show();
