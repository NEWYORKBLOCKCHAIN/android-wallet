...
public class EthereumNetworkRepository extends EthereumNetworkBase
{
    ...
    private void buildPopularTokenMap(List<Long> networkFilters)
    {
        ...
        {
            for (UnknownToken unknownToken: knownContract.getBNB())
            {
                popularTokens.put(unknownToken.address.toLowerCase(), new ContractLocator(unknownToken.address, BINANCE_MAIN_ID));
            }
        }
        if (networkFilters == null || networkFilters.contains(AVALANCHE_ID)){
            for (UnknownToken unknownToken: knownContract.getAVA())
            {
                popularTokens.put(unknownToken.address.toLowerCase(), new ContractLocator(unknownToken.address, AVALANCHE_ID));
            }
        }
        if (networkFilters == null || networkFilters.contains(MATIC_ID)){
            for (UnknownToken unknownToken: knownContract.getMATIC())
            {
                popularTokens.put(unknownToken.address.toLowerCase(), new ContractLocator(unknownToken.address, MATIC_ID));
            }
        }
    }
